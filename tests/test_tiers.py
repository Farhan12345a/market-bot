...
check("tier 1: +1.0% -> 33%", T[0]=={"gain_pct":1.0,"sell_fraction":0.33}, T[0])
check("tier 2: +1.25% -> 40%", T[1]=={"gain_pct":1.25,"sell_fraction":0.4}, T[1])
...
check("tier1 at exactly +1.0%", q==99 and r=="TAKE_PROFIT_1%", (q,r))
check("33% of 300 = 99 shares", q==99)
st2,t2=mk()
q,r=t2.check_take_profit(px(100,1.25))
check("at +1.25% the HIGHEST qualifying tier fires (tier2, not tier1)", r=="TAKE_PROFIT_1.25%", r)
check("tier2 sells 40% of the original = 120", q==120, q)
st3,t3=mk()
...
check("tier1 fired, 99 sold", q==99 and t.qty_remaining==201, (q,t.qty_remaining))
...
check("tier2 fires next, 120 sold", q==120 and r=="TAKE_PROFIT_1.25%", (q,r))
check("81 shares left (300-99-120)", t.qty_remaining==81, t.qty_remaining)
check("tiers 1 and 2 both retired", {0,1} <= t.take_profit_tiers_done)
q,r=t.check_take_profit(px(100,1.6))
check("tier3 sells the remaining 81", q==81 and r=="TAKE_PROFIT_1.5%", (q,r))
...
st,t=mk(3)
q,r=t.check_take_profit(px(100,1.05))
check("33% of 3 -> 0 shares, no order emitted", q==0 and r is None, (q,r))
q,r=t.check_take_profit(px(100,1.6))
check("...but tier3 still closes the 3 shares", q==3, q)
st,t=mk(1)
